...
| `JWT_SECRET` | -- | Секретный ключ для HS256 (обязателен если JWT включён, если не задан `JWT_SECRETS`) |
| `JWT_SECRETS` | -- | Список HS256-секретов через запятую; токен принимается, если подпись сходится с любым из них. Нужен, когда несколько хост-приложений подписывают разными ключами. Приоритетнее `JWT_SECRET` |

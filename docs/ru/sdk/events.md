...
Событие эмитится **синхронно в момент выбора диалога**, до загрузки его сообщений и участников, а не после. Это важно, если вы возвращаете значение обратно в компонент — обычный случай, когда выбранный диалог хранится в URL, а он же передаётся в проп `dialogId`:

```vue
<MTChat :config="config" :dialog-id="dialogIdFromUrl" @dialog-selected="onDialogSelected" />
```

Поскольку событие привязано к выбору, а не к завершению загрузки, вы всегда получаете последний выбор пользователя. Если бы оно эмитилось после загрузки, при быстром переключении диалоги приходили бы в том порядке, в котором завершились их запросы, и устаревшее значение, записанное обратно в `dialogId`, увело бы пользователя из только что открытого диалога.

Обратная сторона: событие уже отправлено, даже если загрузка диалога затем упала (например, его успели удалить). Поэтому URL, обновлённый по этому событию, может указывать на диалог, который не открылся — если это нужно обрабатывать, слушайте `error`.
...
const {
  ...
  isLoading,            // Ref<boolean> — любая загрузка
  isLoadingDialogs,     // Ref<boolean> — только список диалогов
  isLoadingMessages,    // Ref<boolean> — только сообщения открытого диалога

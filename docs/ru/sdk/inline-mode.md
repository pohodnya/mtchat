...
Проп `interceptObjectNavigation` в inline-режиме не действует -- ссылка на объект, которую он контролирует, здесь не рендерится вовсе, так как хост уже находится на странице объекта.

## События

Inline-режим эмитит те же события, что и полный режим (см. [События](events.md#события-компонента)), с двумя исключениями:

- `dialog-selected` не эмитится -- нет списка диалогов для выбора.
- `object-navigate` не эмитится -- ссылка на объект в inline-режиме не показывается (см. заметку про `interceptObjectNavigation` выше).

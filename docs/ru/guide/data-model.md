...
| object_id | STRING | Ссылка на бизнес-объект (макс. 255 символов) |
| object_type | STRING | Тип бизнес-объекта (обязательно) |
| title | STRING | Заголовок диалога (макс. 500 символов) |
| object_url | STRING | Ссылка на объект в приложении (опционально) |
| created_by | STRING | Внешний ID пользователя, создавшего диалог (макс. 255 символов) |
...
| user_id | STRING | Внешний идентификатор пользователя (макс. 255 символов) |
| display_name | STRING | Отображаемое имя (макс. 200 символов) |
| company | STRING | Компания пользователя (макс. 200 символов) |
...
- **user_id** -- внешний строковый идентификатор из вашей системы (напр., `user-123`, `ORG_001`)
...
|------|-----|----------|
| dialog_id | UUID | Ссылка на диалог |
| scope_level0 | STRING[] | Тенанты/организации |
...
| id | UUID (v7) | Уникальный идентификатор, упорядоченный по времени |
| dialog_id | UUID | Ссылка на диалог |
| sender_id | STRING | Внешний ID отправителя (nullable для системных сообщений) |
